...
function extract_region_from_virtualslide($slide_id, $level, $origin_x, $origin_y, $width, $height)
{
    ...
    $query = 'SELECT width AS full_width, height AS full_height, tile_width, tile_height, bgcolor FROM virtualslide_levels JOIN virtualslides USING (slide_id) WHERE slide_id=%d AND level=%d';
    $result = zf_mysql_query($query, $slide_id, $level);
    ...
    */
    ...
    $query = "SELECT SQL_NO_CACHE tile_id, image FROM virtualslide_tiles WHERE (slide_id=%d AND level=%d) AND (tile_id BETWEEN %d AND %d) AND (tile_id %% %d BETWEEN %d AND %d)";
    $result = zf_mysql_query(
        $query,
        $slide_id,
        $level,
        $first_tile,
...
if (array_key_exists('id', $_GET) && ctype_digit($_GET['id'])) {
    ...
    $result = zf_mysql_query($sql['select_label_coords_by_id'], $_GET['id']);
